...
export function CareerFilters({ 
  ...
  const filterOptions: { id: RiasecType | 'all', label: string }[] = [
    { id: 'all', label: 'Todos los Perfiles' },
    { id: 'realistic', label: 'Realista' },
    { id: 'investigative', label: 'Investigativo' },
    { id: 'artistic', label: 'Artístico' },
    { id: 'social', label: 'Social' },
    { id: 'enterprising', label: 'Emprendedor' },
    { id: 'conventional', label: 'Convencional' },
  ]

  const currentLabel = filterOptions.find(opt => opt.id === riasecFilter)?.label || 'Todos los Perfiles'

  return (
    <div className="flex flex-col sm:flex-row gap-6 sm:gap-4 items-center justify-between w-full h-auto sm:h-[80px] mb-8 sm:mb-0">
      
      {/* 3D Static Search Bar */}
      <div className="relative flex-1 group w-full h-[80px] sm:h-full flex z-10">
        ...
        >
          <div className="flex flex-col gap-1">
            {filterOptions.map(option => {
              const isActive = riasecFilter === option.id
              return (
                <button
                  key={option.id}
                  onClick={() => {
                    onRiasecChange(option.id)
                    setIsOpen(false)
                  }}
